<template>
    <div class="bg-white p-10 rounded-xl ring-8 ring-white ring-opacity-40">
        <div class="flex justify-between">
            <div class="flex flex-col">
                <span class="text-6xl font-bold">{{dataForecast.current.temp_c}}°C</span>
                <span class="font-semibold mt-1 text-gray-500">{{ dataForecast.location }} ({{ dataForecast.localTime }})</span>
                <ul class="max-w-md mt-4 space-y-1 text-gray-800 list-none list-inside text-sm">
                    <li>
                        Cảm giác như {{ dataForecast.current.feelslike_c }}°C, {{ dataForecast.current.condition.text }}
                    </li>
                    <li>
                        Tốc độ gió: {{ dataForecast.current.wind_mph }} m/s
                    </li>
                    <li>
                        Độ ẩm không khí: {{ dataForecast.current.humidity }}%
                    </li>
                </ul>
            </div>
            <div class="h-24 w-24 fill-current border-0" title="Free Weather API"><img :src='dataForecast.current.condition.icon' class="h-full" alt="Weather data by WeatherAPI.com"></div>
        </div>
